...
import { Note, SearchResponse } from '@/types';
...
export default function DashboardPage() {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const response = await api.get<SearchResponse>('/search', {
          params: { q: query, mode: searchMode, limit: 20 },
        });
  ...
  const handleDeleteNote = async (noteId: string) => {
    ...
    try {
      ...
      setSearchResults(searchResults.filter((n) => n.shortid !== noteId));
    } catch (error) {
  ...
  const localFilteredNotes = notes.filter((note) =>
    note.title.toLowerCase().includes(searchQuery.toLowerCase())
  );
  const useServerResults = searchQuery.trim().length >= 2;
  const displayedNotes = useServerResults ? searchResults : localFilteredNotes;
  ...
  return (
    <div className="min-h-screen bg-dark-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-dark-400 text-sm mt-1">
              Search runs through the sharded MongoDB cluster.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            <select
              ...
              className="bg-dark-800 border border-dark-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-accent-primary"
            >
        ...
        {searching && (
          <div className="text-sm text-dark-400 mb-4">Searching sharded notes...</div>
        )}
        {searchError && (
          <div className="text-sm text-red-400 mb-4">{searchError}</div>
        )}
        {useServerResults && !searching && !searchError && (
          <div className="text-sm text-dark-400 mb-4">
            {displayedNotes.length} {searchMode} result{displayedNotes.length === 1 ? '' : 's'}
          </div>
        )}
        ...
        {displayedNotes.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-dark-400 mb-6">
              {searchQuery
                ? 'Try a different search term or switch search modes'
                      ...
                      {note.tags && note.tags.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-3">
                          {note.tags.slice(0, 5).map((tag) => (
                            <span key={tag} className="text-xs bg-dark-700 text-dark-300 px-2 py-1 rounded">
                              {tag}
                            </span>
                          ))}
                        </div>
